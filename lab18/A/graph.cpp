...
void Graph::generateDotFile(const std::string filename) const {
    ...
    for (int vertex = 0; vertex < adj_matrix.size(); ++vertex) {
        for (int neighbour = vertex + 1; neighbour < adj_matrix[vertex].size(); ++neighbour) {
            if (adj_matrix[vertex][neighbour] != 0) {
                file << "    " << vertex << " -- " << neighbour << ";\n";
            }
        }
    }
